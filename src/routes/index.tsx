...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Wikrama Hotel Bogor — Penginapan Elegan di Kota Bogor" },
    ],
  }),
  component: Home,
});
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="relative min-h-[92vh] flex items-end pb-32 pt-32">
        <img src={heroImg} alt="Wikrama Hotel Bogor" className="absolute inset-0 w-full h-full object-cover" width={1920} height={1280} />
        <div className="absolute inset-0" style={{ background: "var(--gradient-hero)" }} />
        <div className="relative max-w-7xl mx-auto px-6 w-full">
          <div className="max-w-2xl text-background">
            <div className="text-xs uppercase tracking-[0.3em] text-accent mb-4">Bogor · Sejak 2010</div>
            <h1 className="font-serif text-5xl md:text-7xl leading-[1.05] mb-6">Ketenangan di Tengah Kota Hujan</h1>
            <p className="text-lg text-background/85 max-w-lg leading-relaxed">
              Rasakan kehangatan menginap di Wikrama Hotel — sebuah peraduan elegan dengan sentuhan tropis khas Bogor.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-secondary/30">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
            ...
            <Link to="/rooms" className="text-sm text-primary hover:text-primary/70 underline underline-offset-4 font-medium">
              Lihat semua kamar →
            </Link>
          </div>
                ...
                >
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="bg-[#003b95] text-white text-[9px] font-bold px-1.5 py-0.5 rounded-sm uppercase tracking-wider ml-1">Genius</span>
                    </div>

                    <h3 className="font-bold text-lg leading-tight mb-1 group-hover:text-primary transition-colors">{r.name}</h3>
                    <div className="text-xs text-muted-foreground mb-3">Bogor, Indonesia</div>

                    <div className="flex items-center gap-2 mb-3">
                      <div className="bg-[#003b95] text-white font-bold p-1.5 rounded-sm rounded-tl-md rounded-br-md text-sm flex items-center justify-center min-w-[32px]">{r.ratingScore}</div>
                      <div className="flex flex-col">
                        <span className="text-xs font-bold leading-none">{r.ratingWord}</span>
                        <span className="text-[10px] text-muted-foreground mt-0.5">{r.reviews} ulasan</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto flex flex-col items-end text-right border-t border-border pt-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="line-through text-red-500/80 text-xs font-medium">{formatIDR(r.originalPrice)}</span>
                        <span className="text-xl font-bold text-foreground">{formatIDR(r.price)}</span>
                      </div>
                    </div>
                  </div>
                </Link>
