    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          <Link to="/" className="text-sm font-medium hover:text-primary transition-colors text-foreground/70 [&.active]:text-primary">
            Beranda
          </Link>
          <Link to="/rooms" className="text-sm font-medium hover:text-primary transition-colors text-foreground/70 [&.active]:text-primary">
            Kamar
          </Link>
          ...
          <Link to="/about" className="text-sm font-medium hover:text-primary transition-colors text-foreground/70 [&.active]:text-primary">
            Tentang
          </Link>
          <Link to="/contact" className="text-sm font-medium hover:text-primary transition-colors text-foreground/70 [&.active]:text-primary">
            Kontak
          </Link>
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {user && (
            <button onClick={handleLogout} className="text-muted-foreground hover:text-red-500" title="Logout">
              <LogOut size={20} />
            </button>
          )}
        </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-background border-b border-border p-6 flex flex-col gap-5 animate-in fade-in slide-in-from-top-2 shadow-lg absolute w-full">
          <Link to="/" onClick={() => setIsMenuOpen(false)} className="text-lg font-serif">Beranda</Link>
          <Link to="/rooms" onClick={() => setIsMenuOpen(false)} className="text-lg font-serif">Kamar</Link>
          
          {/* MENU CAFE MOBILE */}
          <Link to="/cafe" onClick={() => setIsMenuOpen(false)} className="text-lg font-serif flex items-center gap-2">
            Cafe & Resto <Coffee size={16} className="text-orange-500" />
          </Link>
          
          <Link to="/about" onClick={() => setIsMenuOpen(false)} className="text-lg font-serif">Tentang</Link>
          <Link to="/contact" onClick={() => setIsMenuOpen(false)} className="text-lg font-serif">Kontak</Link>

          {/* TOMBOL FAVORIT (MOBILE) */}
          <Link to="/favorites" onClick={() => setIsMenuOpen(false)} className="text-lg font-serif flex items-center gap-2 text-red-500">
            Favorit Tersimpan 
          ...
          {!user ? (
            <Link to="/login" onClick={() => setIsMenuOpen(false)} className="text-lg font-serif flex items-center gap-2">
              <User size={18} /> Login
            </Link>
          ) : (
            <button onClick={() => { handleLogout(); setIsMenuOpen(false); }} className="text-lg font-serif flex items-center gap-2 text-left text-red-500">
